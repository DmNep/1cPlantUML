...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия 
	ВерсияОбработки = "v.0.05 20-07-2026";  
...
Функция СоздатьСхемуPlantUML()
	...
	Для Каждого СтрокаОбмены Из Объект.Обмены Цикл
		...
		Если ЗначениеЗаполнено(СтрокаОбмены.КомментарийПравый) Тогда
			СтрокаМассив = СтрРазделить(СтрокаОбмены.КомментарийПравый, "|");
			СтрокаКомментарий = "";
			Для Каждого ЭлементМассива Из СтрокаМассив Цикл
				СтрокаКомментарий = СтрокаКомментарий + ЭлементМассива + Символы.ПС;	
			КонецЦикла;
...
КонецФункции  
//16-12-2024 v0.04 Добавлено оформление ТЧ. Выделение полужирным шрифтом и и фоном разделителей. 
...
Процедура ЗаписьВ_XML(ПутьКФайлу)
	...
	ТекстКомментария = ТекстКомментария + "'v.0.05  20-07-2026" + Символы.ПС;
